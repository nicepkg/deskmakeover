...
// the canvas wallpaper scene, the real-icon mock pack (public/mock-icons/,
// generated by scripts/dev/generate-mock-icons.mjs) and the session state
...
// ---- desktop items: REAL icon pack first, synthetic fallback ----
// The REAL pack (public/mock-icons-real/, gitignored dev fixtures harvested by
// scripts/dev/fetch-real-icons.mjs — owner order 2026-07-09) is the default:
// genuine Windows system icons + real app icons at their native sizes. The
// committed synthetic pack remains the fallback for a fresh clone that has
// not run the fetch script yet.
...
async function loadManifest(): Promise<PackEntry[]> {
  manifestPromise ??= (async () => {
    const real = await fetch('/mock-icons-real/manifest.json').catch(() => null)
    if (real?.ok) {
      ...
      const all: PackEntry[] = entries.map((e) => ({
        ...
        label: e.label,
        sourceUrls: [
          `/mock-icons-real/${e.file}`,
          ...(e.extraSources ?? []).map((f) => `/mock-icons-real/${f}`),
        ],
      }))
      const scenario = currentScenario()
      if (scenario !== 'messy') {
    ...
    console.info('[mock desktop] real pack missing — synthetic fallback (run scripts/dev/fetch-real-icons.mjs)')
    const entries = (await fetch('/mock-icons/manifest.json').then((r) => r.json())) as SyntheticEntry[]
    return entries.map((e) => {
      const kind = KIND_MAP[e.kind]
      const url = `/mock-icons/${e.file}`
      return { id: e.id, kind, label: e.label, sourceUrls: kind === 'RecycleBin' ? [url, url] : [url] }
    })
  })()
