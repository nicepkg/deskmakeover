...
3. **Apply is irreducibly C#**: IShellLink COM (.lnk), INI (.url), desktop.ini+attrs (folder), wrapper-.lnk+hide (loose exe/file), HKCU CLSID registry (Recycle Bin), elevated helper (arrow overlay). All with full restore metadata. ~~UWP/MSIX icons have NO safe reversible override~~ **CORRECTED 2026-07-09: wrong.** The owner's prototype proves UWP desktop `.lnk`s take a standard IconLocation write (the immutable thing is the package asset, not the shortcut); AppxShortcut is styleable and joins CanStyle in the Windows batch. ICO assembly tested in C# (`IcoWriter`, uncompressed DIB frames).
