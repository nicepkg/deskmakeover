...
- **Icons**: the Rust `dm-icon-core` renders every preview tile AND the bake master — compiled to
  **WASM** for the web preview + foreground/manual bake, and to **native** for the resident/background
  path. Foreground apply = WASM bakes the masters, then the Rust host packages + writes them (no
  native re-render); `icons.switchVersion` likewise selects a persisted version and writes it. The
  frozen TS `src/icon-compositor/` is the
...
- **WYSIWYG holds by construction**: the web preview AND the manual bake both run the WASM
  `dm-icon-core`; the resident/background renderer is the *same Rust core's native build*, held
...
| `wallpaper.*` | `getScreens` (thin `WallpaperScreensDto` + source URLs) · `applyBaked(monitorId, png)` · `restore(monitorId)` — source pixels are fetched over the `dmwallpaper://` protocol, NOT an invoke verb (setLook is frontend) |
